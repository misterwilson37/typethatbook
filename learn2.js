...
// lesson's FINAL run can be Deadline (game-deadline.js) instead of a typed
// drill, and it is a fork — not a flag inside learn.js — because Deadline's
// keystrokes never touch drillPos/learnLastInputTime, so the graded-time gate
// four counting bugs died to make singular (tests/open-unit-test.mjs Part E,
// which reads `../learn.js` by hardcoded filename and fails if
// `statsData.secondsToday++` appears more than once in THAT file) cannot see
// a game run without a second increment site inside learn.js itself — the
// exact shape that harness exists to catch. Forking to a file the harness has
// never heard of was Jake's call, 2026-09-09, over changing the harness or the
// page every student is on today.
//
// ⚠️⚠️ WHAT THAT MEANS FOR RULE 9: this file carries its OWN full copy of the
// grading/time engine, including a second, clearly-labelled time-increment
// site (bankGameSecond(), below) that exists ONLY in this fork. That is a
// deliberate, temporary exception — the same role tools/game-lab.html and
// arcade.html (before Deadline was wired) already play in this repo: real and
// playable, NOT the page a whole roster is on. Deliberately ABSENT from
// versions.js, same as the arcade was kept out while inert (Round 82's
// ruling). It MUST be reconciled — promoted to replace learn.html, or folded
// back into learn.js behind a flag — before this goes near a whole class.
// Everything below this banner that is not called out as new is learn.js
// v2.48.0's own code, unchanged; see that file for the full prior history.
//
// ═════════════════════════════════════════════════════════════════════════
// WHAT'S ACTUALLY NEW (search these names to find every touch point):
//   isGameRun()        — decides whether a run index is the game slot
//   beginGameStep()     — mounts Deadline instead of the typed drill
//   bankGameSecond()    — the second time-increment site (see banner above)
//   finishGameStep()    — the game's tail, mirroring finishStep()'s
//                         isLastRun-true branch with the game's own numbers
//   destroyGameHandle() — torn down at every exit this file already has
//                         (beginStep, stopLesson, restartLesson)
//   gameHandle          — module-level, holds mountDeadline()'s handle
// ═════════════════════════════════════════════════════════════════════════
//
...
// ⚠️ readOneDeployedVersion NOT IMPORTED — learn2.html/learn2.js are
// deliberately absent from versions.js, so nothing here can ask it to look
// them up. See the header banner.
...
// ⚠️⚠️ NEW IN learn2.js — SEE THE BANNER AT THE TOP OF THIS FILE.
// missionConfigFromRun() is the SAME function arcade.html already calls to
// build a Deadline mission from a run — reusing it, not reproducing its
// arithmetic, is what keeps a run played here comparable to one played there.
// mount() (aliased mountDeadline, matching arcade.html's own alias) is
// game-deadline.js's only public entry point.
...
// ⚠️ NOT versions.js-REGISTERED — see the header banner. This is a staging
...
// ⚠️⚠️ NEW IN learn2.js — see the header banner. Holds mountDeadline()'s
// handle for the currently-mounted game run, or null when none is mounted.
// destroy()ed at every exit this file already had before the fork existed
// (beginStep() moving to a different run, stopLesson(), restartLesson()) —
// see destroyGameHandle(), called from all three.
let gameHandle     = null;
...
function restartLesson() {
    if (!currentLesson) return;
    destroyGameHandle();   // ⚠️ NEW IN learn2.js — see the header banner.
    clearInterval(timerInterval);
    clearInterval(learnTickInterval);
    stopIntroAnim();
...
function showIntro(lesson) {
    ...
    // ⚠️ NEW IN learn2.js — the run picker. Drawn AFTER startLesson() has set
    // currentStepIdx (via firstOpenRunIdx()), so the default selection is the
    // lesson's own idea of where to start and the picker only ever offers to
    // change it. See renderRunPicker()'s header for why the default is
    // untouched.
    renderRunPicker(lesson);
    syncIntroStartLabel();
...
function beginStep(stepIdx) {
    ...
    // ⚠️⚠️ NEW IN learn2.js — see the header banner and isGameRun()'s own
    // comment. Any earlier mount (Game Genie jump, a retry that lands back
    // here) is torn down before this function does anything else, same as
    // stopLesson()/restartLesson() below.
    destroyGameHandle();
    if (isGameRun(stepIdx)) {
        beginGameStep(stepIdx);
        return;
    }
...
// ═════════════════════════════════════════════════════════════════════════
// ⚠️⚠️ EVERYTHING BELOW TO THE NEXT BANNER IS NEW IN learn2.js.
// See the header comment at the top of this file before changing any of it.
// ═════════════════════════════════════════════════════════════════════════

// Is `idx` the game slot? Only the TRUE final run of a REAL lesson — never a
// remediation drill (currentRunsFor === REMEDIATION_RUNS while one is active,
// so this reads false automatically without checking that flag directly) and
// never a practice replay (practiceRun, set by armRunMode() just before this
// is ever consulted — a practice run writes nothing anywhere, and a game that
// FEELS graded while writing nothing would be its own kind of dishonest).
// Both exclusions fall back to the ordinary typed drill, which is the safe
// default for anything that isn't explicitly the lesson's graded last run.
//
// ⚠️ TWO ADDITIONAL GATES, JAKE'S RULING 2026-09-09.
//
//   • A ONE-RUN LESSON IS NEVER A GAME. *"If there were only one run, it
//     should not be a game."* Otherwise the whole lesson would be the game
//     and the student would never type the drill at all. ⚠️ THIS IS LIVE, NOT
//     HYPOTHETICAL: six authored lessons (u7_p1–p4, p7, p8) are a single short
//     sentence_list that chunks to one run.
//   • ⭐ NO GAME WHERE SPEED IS NOT GRADED. Jake's ruling, 2026-09-09: *"I
//     would lean toward no game on any lesson where wpm doesn't matter."*
//     ⚠️⚠️ THIS IS THE GATE THAT MATTERS MOST AND IT IS NOT A PREFERENCE — IT
//     CLOSES AN INCOHERENCE. A run whose gates carry `minWPM: null` is graded
//     on accuracy ALONE (run-grade.js calculateGrade()'s accuracy-only
//     branch), but a Deadline mission is a THROUGHPUT test by construction:
//     the city falls if you cannot keep up. So on such a run the game would
//     decide pass/fail on a quantity the grade formula is forbidden to look
//     at — the student loses their city for being slow and is then told speed
//     was never being measured. Whichever way that resolves, one of the two
//     screens is lying to them.
//     ⚠️ IT ALSO REMOVES THE `missionConfigFromRun()` FALLBACK FROM THE GRADED
//     PATH ENTIRELY. game-shell.js's header: an accuracy-only run *"must not
//     be offered as a substitute ... without Jake's sign-off"*, because the
//     game has to invent a pace the typed run never had. Nothing invents a
//     pace now — every game run reads a real authored gate.
//     ⚠️ WHICH LESSONS THIS EXCLUDES, AND WHY THEY ARE NOT A CORPUS BUG:
//     u1_l1–u1_l5, u2_l1–u2_l5 and u5_l1–u5_l5 all end on a key_random step,
//     which DRILL_TYPES forces to minWPM null. Units 1 and 2 because a
//     handful of letters cannot spell a word; **Unit 5 because it is the
//     NUMBER ROW and digits cannot spell one either** — u5_l1–l5 are literally
//     pattern → random → combined-digits, with no prose step available to
//     them. It is the same structural cause, not an authoring slip.
//     ⚠️ u5_l6 KEEPS ITS GAME: the Number Row Synthesis capstone ends on a
//     mixed-alphanumeric sentence_list, so it carries a real speed gate.
//   • AT LEAST GAME_MIN_KEYS CUMULATIVE KEYS. *"Spamming the game is lame and
//     not worth doing."* With two keys there is nothing to spell.
//     ⚠️ THIS GATE CANNOT FIRE AGAINST THE CURRENT CORPUS AND IS KEPT
//     DELIBERATELY. The only lesson under 4 cumulative keys is u1_l1, which
//     the speed-gate rule above already excludes. It is retained because it
//     encodes a SEPARATE intent — "do not game a two-key lesson" — that would
//     otherwise be enforced only by coincidence, and a future authored lesson
//     that put a word_list on a tiny key set would find it waiting. ⚠️ IF YOU
...
// including) — two different windows over the same list is how the arcade
// would end up drawing letters from one lesson and its gate from another, and
// game-shell.js's header calls that out explicitly. Same reasoning here.
function cumulativeKeyCount(lesson) {
    const idx = allLessons.findIndex(l => l.id === lesson.id);
    if (idx < 0) return 0;
    const keys = new Set();
    for (let i = 0; i <= idx; i++) {
        (allLessons[i].newKeys || []).forEach(k => keys.add(k));
    }
    return keys.size;
}
...
//
// ⚠️⚠️ EVERY SURFACE ASKS THIS FUNCTION AND NONE OF THEM RE-DERIVES IT. The
// first draft of this fork spelled the eligibility test out THREE times — in
// isGameRun(), in renderRunPicker() (to draw the 🚀 chip) and in
// syncIntroStartLabel() (to write "Play Deadline") — which is Rule 9 in its
// smallest form: three copies of one rule, and the failure mode is a picker
// that promises a rocket the lesson then declines to deliver, or a Start
// button that says "Run 6" and launches a game. A student would report that as
// the app lying to them, and all three copies would individually look correct.
//
// ⚠️ IT DELIBERATELY DOES NOT CONSIDER practiceRun / mastery. That is a
// property of an ATTEMPT, not of the lesson's shape, and isGameRun() layers it
// on top — see there. A caller that wants to know "would clicking this launch
// the game" must ask both, which renderRunPicker() does via modeForRun().
...
function isGameRun(idx) {
    return !practiceRun && idx >= 0 && idx === gameSlotIdx();
}

function destroyGameHandle() {
    if (gameHandle) {
        try { gameHandle.destroy(); } catch (e) { console.warn('[TTB] game destroy() threw:', e); }
        gameHandle = null;
    }
...
// IN THIS FORK. See the header banner: Deadline's keystrokes never touch
// drillPos/learnLastInputTime, so startGradedTimer()'s gate can never see
// them, and there is no way to bank a game run's seconds through the site
// four counting bugs died to create. This is that site's twin for the one new
// modality this file adds — same statements, same order, called once per
// WHOLE ELAPSED GRADED SECOND exactly as game-deadline.js's mount() defines
// onSecond: never before the first keystroke, never while paused (including a
// hidden tab — game-deadline.js watches document.hidden itself), never after
// the run ends. It is not a THIRD record of a quantity that already exists —
// it is the SAME counters, incremented from a second doorway because a game
// has no keystroke for the first doorway to gate on.
// ⚠️ KEEP THIS IN THE SAME ORDER AS startGradedTimer()'s tick body. The two
// are twins, not independent — a change to one's ordering (the rollover above
// the increments, anonSecondsAccum above armAnonLoginPrompt()) belongs in both
// or neither.
function bankGameSecond() {
    rollDayIfNeeded('tick');
    stepSeconds++;
    learnActiveSeconds++;
    statsData.secondsToday++;
...
// ═════════════════════════════════════════════════════════════════════════
// THE RUN PICKER — Jake's report, 2026-09-09
// ═════════════════════════════════════════════════════════════════════════
//
// ⚠️⚠️ THE BUG BEHIND THIS WAS NOT WHAT EITHER OF US FIRST THOUGHT, AND THE
// CORRECTION MATTERS MORE THAN THE FEATURE. Jake: *"a student got kicked back
// to the first run and had no way to move forward with it."*
//
// Earlier in this same conversation I told him "start on run 5, come back
// later" already worked, via Round 34's firstOpenRunIdx(). **THAT WAS WRONG
// FOR THE ORDINARY STUDENT AND HIS REPORT IS THE PROOF.** firstOpenRunIdx()
// returns the first run whose mode is not 'practice', and a run only becomes
// 'practice' once it is MASTERED — MASTERY_POINTS = 4, banked at A🔥 = 2 and
// A = 1, with B, C, D and F worth ZERO (lesson-gate.js RUN_POINTS). So the
// resume only ever fires for a student who scored two fireballs, or a fireball
// and two A's, or four A's, ON EVERY EARLIER RUN. A child who passed runs 1–4
// with B's and C's — the normal case, the case the gates are tuned for —
// banks nothing, and every visit reopens the lesson at run 1.
//
// ⚠️ SO THE STUDENT WAS NOT KICKED BACK BY A DEFECT. Round 34 fixed a real
// problem ("the gate was taxing the student it meant to move along") and fixed
// it only for students who were already excelling. Nothing is broken in
// firstOpenRunIdx(); it simply never covered the case Jake watched happen.
//
// ⚠️ AND IT SITS AGAINST A STANDING RULING, WHICH IS WHY THIS IS A PICKER AND
// NOT A NEW AUTO-RESUME. Jake, 2026-08-17, the ruling that deleted mid-lesson
// resume in v2.5.0: *"If a kid doesn't finish a lesson one session, they should
// restart it — not start at the last word. The lesson should be taken as a
// whole."* That ruling is about the DEFAULT and about resuming mid-RUN, at the
// character. This changes neither: the lesson still opens where it always did
// (firstOpenRunIdx()), and a run still always begins at character zero. What
// is new is that the student may CHOOSE a run, deliberately, from a screen —
// which is also the "more practice where necessary" half of Jake's ask.
//
// ⚠️⚠️ THE CEILING IS THE POINT, AND WITHOUT IT THIS FEATURE HANDS EVERY
// STUDENT THE WHOLE CURRICULUM. A lesson is marked passed by
// showLessonResultModal(), which only the FINAL run reaches — so an
// unrestricted picker would let a child open any lesson, jump straight to the
// last run, and pass it without typing a character of runs 1..N-1. Now that
// the last run is the game, that is also a one-click route to the only fun
// thing on the page, on repeat, forever.
//
// So: a student may replay anything they have already reached, and may step
// exactly ONE run past it — which is the next run they would have arrived at
// by typing anyway. `furthestRunIdx` is already on the progress record
// (recordRunOutcome() maintains it as a running max) so this costs no new
...
function renderRunPicker(lesson) {
    const host = document.getElementById('intro-run-picker');
    if (!host) return;
    host.innerHTML = '';
    // ⚠️ ONE RUN MEANS NO CHOICE TO OFFER. A picker with a single chip is
    ...
    label.style.cssText = 'font-size:0.78rem;color:#888;margin:10px 0 5px;' +
        'font-family:\'Courier Prime\',monospace;text-align:center;';
    label.textContent = maxIdx > 0 ? 'Start from which run?' : '';
    host.appendChild(label);

    const row = document.createElement('div');
    row.style.cssText = 'display:flex;flex-wrap:wrap;gap:6px;justify-content:center;' +
        'margin-bottom:10px;';
    ...
    currentRuns.forEach((run, i) => {
        ...
        const mastered = runMastered(userProgress[lesson.id], i);
        // ⚠️ BOTH HALVES. gameSlotIdx() says the lesson HAS a game there;
        // modeForRun() says whether THIS student's attempt would be graded.
        // A mastered final run replays as ordinary typed practice, so drawing
        // a rocket on it would promise something that will not happen.
        const isGame = i === gIdx && modeForRun(lesson, i) !== 'practice';
        chip.textContent = isGame ? '\uD83D\uDE80' : String(i + 1);
        chip.title = isGame
            ? 'Final run \u2014 Deadline' + (reachable ? '' : ' (finish the earlier runs first)')
            : reachable
                ? (mastered ? 'Run ' + (i + 1) + ' \u2014 mastered, replay for practice'
                            : 'Run ' + (i + 1))
                : 'Run ' + (i + 1) + ' \u2014 finish the earlier runs first';
        chip.disabled = !reachable;
        chip.style.cssText =
            'min-width:34px;height:34px;padding:0 8px;border-radius:5px;cursor:' +
            (reachable ? 'pointer' : 'not-allowed') + ';' +
            'font-family:\'Courier Prime\',monospace;font-size:0.85rem;font-weight:bold;' +
            'border:1px solid ' + (i === currentStepIdx ? 'var(--carolina-blue,#4B9CD3)' : '#555') + ';' +
            'background:' + (i === currentStepIdx ? 'var(--carolina-blue,#4B9CD3)' : 'transparent') + ';' +
            'color:' + (i === currentStepIdx ? '#fff' : reachable ? (mastered ? '#7ba05b' : '#bbb') : '#555') + ';' +
            'opacity:' + (reachable ? '1' : '0.45') + ';';
        if (reachable) {
            chip.onclick = () => {
                currentStepIdx = i;
                renderRunPicker(lesson);        // repaint the selection
                syncIntroStartLabel();
            };
        }
    ...
    if (maxIdx < currentRuns.length - 1) {
        const note = document.createElement('div');
        note.style.cssText = 'font-size:0.72rem;color:#777;text-align:center;' +
            'margin-bottom:8px;font-family:\'Courier Prime\',monospace;';
        note.textContent = 'Finish a run to unlock the next one.';
        host.appendChild(note);
    }
}

// The Start button names the run it will actually start, because after a pick
// "Start Lesson →" would be a lie about where the student is going.
function syncIntroStartLabel() {
    if (!introStartBtn) return;
    if (!currentRuns.length || currentRuns.length < 2) {
        introStartBtn.textContent = 'Start Lesson \u2192';
        return;
    }
    // ⚠️ ASKED, NOT RE-DERIVED — see gameSlotIdx()'s header. Both halves, for
    // the same reason renderRunPicker() checks both.
    const isGame = currentStepIdx === gameSlotIdx() &&
                   currentLesson &&
                   modeForRun(currentLesson, currentStepIdx) !== 'practice';
    introStartBtn.textContent = isGame
        ? '\uD83D\uDE80 Play Deadline \u2192'
        : 'Start Run ' + (currentStepIdx + 1) + ' \u2192';
}

// Mirrors the parts of beginStep() every run needs (hide intro, unhide
// #active-drill, reset the per-run counters, paint the pips/label) and skips
// the parts that only make sense for a typed drill (drillSequence, the
// keyboard's keydown handler, startGradedTimer()'s DOM-keystroke-gated tick).
function beginGameStep(stepIdx) {
    introPanel.classList.add('hidden');
    activeDrill.classList.remove('hidden');
    activeDrill.style.position = '';
    activeDrill.style.top = '';
    ...
    // The typed-drill furniture has no role here — HIDDEN, not removed, so a
    // later run (a retry lands back on THIS run, but a fresh lesson start
    // does not) hands the typed path back an unmodified DOM.
    const drillTextArea = document.getElementById('drill-text-area');
    if (drillTextArea) drillTextArea.style.display = 'none';
    ...
    const anchorHint2 = document.getElementById('anchor-hint');
    if (anchorHint2) anchorHint2.style.display = 'none';
    drillKeyboard.onkeydown = null;

    // Progress pips — identical to beginStep()'s, so the run count reads the
    // same regardless of which slot is a game.
    stepProgressEl.innerHTML = currentRuns.map((r, i) => {
        const cls = i < stepIdx ? 'done' : i === stepIdx ? 'active' : '';
        return `<div class="step-pip ${cls}"></div>`;
    }).join('');
    stepLabelEl.textContent = '\uD83D\uDE80 Final Run \u2014 Deadline';
    ...
    // shape buildRunList() already produces (run.sequence is the run's own
    // baked character array — see buildRunList()'s header). If this page and
    // arcade.html ever disagreed about difficulty they would not be
    // comparable, so this must never hand-build a config.
    ...
    gameHandle = mountDeadline(gameMount, {
        ...
        // ⚠️ SAME AS ABANDONING ANY OTHER RUN MID-WAY (the "← Map" button
        // elsewhere in this file): no grade, no logRun() — just back to the
        // map. stopLesson() already banks whatever seconds bankGameSecond()
        // accrued via saveStats(); nothing here needs to duplicate that.
        onQuit: () => { destroyGameHandle(); stopLesson(); },
    });
}

// The game's own tail, mirroring finishStep()'s isLastRun-true branch. This
// run is ALWAYS the lesson's last — beginGameStep() only ever mounts here —
// so unlike finishStep() there is no showStepModal() branch to consider.
//
// ⚠️⚠️ THE QUOTA GATE IS NOT A POLISH ITEM — IT CLOSES A LESSON-SKIPPING
// EXPLOIT THIS WIRING CREATED. Found by reading calculateGrade() against a
// zero-effort game, 2026-09-09, BEFORE this shipped:
//
//   A student who mounts the game and TYPES NOTHING loses every shield to
//   leaks. game-shell.js does not charge a leak as an accuracy error (Jake's
//   ruling, 2026-09-07, and it is right for a typed-run-shaped measurement),
//   so the report reads chars: 0, mistakes: 0 — and accuracyPct(0) returns
//   100 by definition. Handed to showLessonResultModal() that is acc 100,
//   wpm 0, which calculateGrade() turns into:
//     • an accuracy-only final run (all of Units 1, 2 and 5) → `clean` is
//       true → **A🔥**;
//     • a speed-graded final run → accuracy met, speed short → **'C'**, and
//       gradeAdvances('C') is TRUE because nothing in the corpus sets
//       strictSpeed.
//   Either way saveProgress(passed) marks the LESSON PASSED for a child who
//   sat and watched their city burn.
//
...
// abandoned typed run produces NO outcome — only banked time. The game is the
// first run in this app that can END BY ITSELF, so it is the first one that
// needed this said out loud.
//
// ⭐ SO A LOST GAME IS AN UNFINISHED RUN, NOT A FAILED ONE, and it is treated
// exactly as an abandoned typed run already is: the minutes are banked and the
// sprint is logged (the child really did type for those minutes and Jake's
// standing rule is that time counts whether the work finishes or not), but
// recordRunOutcome() and saveProgress() are never reached, so no grade and no
// advancement are written. ⚠️ DO NOT "FIX" THIS BY INVENTING A GRADE — writing
// 'F' would be a claim about how well they typed, and a student who cleared 90%
// of the mission at 100% accuracy did not earn an F. The honest record is that
// the run did not finish.
function finishGameStep(rep) {
    destroyGameHandle();
    clearInterval(timerInterval);
    clearInterval(learnTickInterval);
    const spaceHint = document.getElementById('space-hint');
...
// GRADED MODAL. Practice says "nothing was recorded", which would be false —
// the minutes were recorded, in both records. The graded modal would imply an
// outcome that deliberately was not written. This says the true thing: the run
// did not finish, the time counted, try it again.
function renderGameLostResult(rep) {
    const title = document.getElementById('dm-title');
    if (title) title.textContent = '\uD83C\uDFD9\uFE0F The City Fell';
    ...
    if (stats) stats.innerHTML =
        '<div class="dm-stat"><div class="dm-val">' + rep.wpm + '</div><div class="dm-label">WPM</div></div>' +
        '<div class="dm-stat"><div class="dm-val">' + rep.acc + '%</div><div class="dm-label">Accuracy</div></div>' +
        '<div class="dm-stat"><div class="dm-val">' + (rep.targetsCleared || 0) + '</div>' +
        '<div class="dm-label">Words cleared</div></div>';

    const msg = document.getElementById('dm-msg');
    if (msg) msg.innerHTML =
        '<div style="font-size:0.85rem;color:#555;">You did not clear the run this time, so it is not ' +
        'graded \u2014 but your time counted. Have another go.</div>' +
    ...
    if (btns) {
        ...
        mapBtn.className = 'dm-btn-secondary';
        ...
        retryBtn.className = 'dm-btn-primary';
        retryBtn.textContent = 'Try Again (Enter)';
        retryBtn.onclick = () => {
            document.removeEventListener('keydown', enterHandler);
            drillModal.classList.add('hidden');
            beginStep(retryIdx);
...
// ═════════════════════════════════════════════════════════════════════════
// END OF WHAT'S NEW IN learn2.js. Everything below is learn.js v2.48.0,
// unchanged, except for the destroyGameHandle() calls added to stopLesson()
// and restartLesson() so a mounted game is never left running underneath a
...
async function updateVersionFooter() {
    ...
    // ⚠️ learn2.html/learn2.js are NOT in versions.js — see the header banner —
    // so readOneDeployedVersion() cannot answer for them the way it does for
    // learn.html. The footer says so plainly rather than showing a confident
    // '?' that reads like a fetch failure.
    const styleVer = readAppliedCssVersion('::before');

    primary.innerHTML =
        `<span style="opacity:.7">learn2.html/js</span> v${LEARN_VERSION} ` +
        `<span style="opacity:.6">(STAGING FORK — not versions.js-registered)</span>` +
...
function _renderFullBuildPanel(results) {
    ...
    let html = `<div style="opacity:.6;margin-bottom:4px">learn2.html (this page — STAGING FORK)</div>`
...
document.title = 'TypeThatBook — School v' + LEARN_VERSION + ' (learn2 staging)';
