# HANDOFF — Rounds 1–3 (Underwood · Dvorak · Blick) — ARCHIVED, STILL USEFUL

> **Round 6 (Noiseless) note.** Jake spent an entire session recovering this file.
> It was `HANDOFF.md` v4.2.0 at the end of Round 3 and was overwritten in place by
> Round 4, so **Rounds 1–3 existed nowhere in the repo.** Committed here so that
> cannot happen a third time. The current handoff is `HANDOFF.md`; Round 5's is
> `HANDOFF-round5.md`.
>
> **Round 4 (Oliver) is genuinely gone.** Mignon confirmed he invented a reference to
> `HANDOFF-round4.md` and never wrote it. Round 4's work survives only as summarised
> in Round 5's handoff and in `CHANGELOG.md`. Don't go looking.
>
> ### Why this file is worth keeping rather than just filing
>
> - **§10 is the definitive missing-document map**, written after Jake went looking
>   for `TTL-GUIDE.md` and couldn't find it. It names seven documents, says what was
>   in each, and rates each one's recoverability. It is the answer to a question
>   Round 6 was about to ask Jake to chase down. **Read §10 before hunting for any
>   document.**
> - **§11 is what `PEDAGOGY-AUDIT.md` §4 used to point at** and the pointer had gone
>   dead. It independently confirms Batch B shipped, with the same field names Round 6
>   documented (`runAttempts` / `runFailures` / `furthestRunIdx` / `lastSeenAt`), and
>   it records two latent bugs fixed on the way that appear nowhere else: `saveProgress()`
>   used `setDoc` **without merge**, and `timeSpentSeconds` counted only the final run.
> - **§3 (hard constraints), §5 (the bootstrap chicken-and-egg, permanent by design),
>   §7 (version constants), §12 (Jake's working preferences)** are Underwood's Round 1
>   text, verbatim, and are the origin of conventions later rounds inherited without
>   knowing where they came from.
>
> ### What in here is now WRONG
>
> - **Every version number.** This is Round 3; the project is on Round 6.
> - **§2's deployment status** — verified 2026-07-31, meaningless now.
> - **§9's open-work list** — superseded by Round 5 §4 and Round 6 §4.
> - **§10's own entry for `TTL-GUIDE.md`** says it is in the repo. It was written,
>   then never committed, and was recovered in Round 6. It is in the repo *now*.
> - **§10's entries for `SCALE-PLAN.md` and `MULTITENANCY.md`** say missing. Both were
>   recovered in Round 6 and are in the repo — but read each one's Round 6 header
>   first, especially `MULTITENANCY.md`, which §10 correctly calls superseded.
...
### A.22 EPUB import: ask the book (admin.js 3.11.0)

Jake sent a Project Gutenberg epub — *Toby Tyler* — and asked the right question:
does this help the algorithm, or would fixing it for this break Aesop?

**Neither. It replaced the algorithm with something better.** v3.9.0 split on
`<article>`/`<section>` containers, which is Standard Ebooks' markup. Gutenberg
uses none: 20 chapters as bare `<h2>` in two 150KB files. The container
heuristic declines and it imports as two enormous chapters.

The tempting fix — "also split at headings" — inherits false positives. A
chapter containing subheadings shatters into fragments. So instead
`chapterUnitsFromToc()` reads the nav document and splits at the anchors **the
book itself declares**. That's not a guess.

⚠️ **Strategy order is TOC → containers → whole file, and the order is verified,
not assumed.** SE's TOC anchors point at exactly the `<article>` elements the
container route found, so Aesop produces the identical 284 fables either way. If
you reorder these, re-run that number.

One document-order pass serves both shapes: a container anchor holds its
paragraphs, a bare-heading anchor is followed by them, and assigning each `<p>`
to the most recent anchor seen is right in both cases. Front matter in the TOC —
title page, Contents — has no paragraphs and drops out with no special case.

⚠️ **`stripBoilerplate()` removes REGIONS, never whole files.** The first version
skipped any spine file containing Gutenberg boilerplate — and Gutenberg puts its
header in the *same file* as chapters I-X, so Toby Tyler silently imported
XI-XX and lost half the book. Caught only because the test asserted the chapter
count against the real file. **Assert counts, not "it ran."**

Verified: Toby Tyler 20/20 with correct titles, no Contents page, no licence;
Aesop's 284 fables unchanged including the cleaned "The Donkey, the Rooster, and
the Lion".

### A.23 The phantom documents, part two (2026-08-02)

Jake went looking for `TTL-GUIDE.md`, `MULTITENANCY.md`, `SCALE-PLAN.md` and
`HANDOFF-round4.md`. Verified answers:

- `TTL-GUIDE.md` — **exists**, written from scratch this session.
- `MULTITENANCY.md`, `SCALE-PLAN.md` — never committed. §10 records what was in
  them and what survived.
- `HANDOFF-round4.md` — **referenced by nothing in this repo.** Not a lost file;
  a name that never appeared here. This project is on Round 3.

⚠️ **§A.-whatever fixed the document MAP and left every inline POINTER dangling.**
Ten of them, including three in `game.js` ("see MULTITENANCY.md", "See
SCALE-PLAN.md"). Correcting a table of contents is not the same as correcting the
references, and I made exactly that mistake after writing a rule about it.

All ten now point at surviving content — `HANDOFF.md` §2/§4 for tenancy and
rules, `TTL-GUIDE.md` for retention cost — or state plainly that the target isn't
in the repo. The one changelog citation is kept verbatim with a note, because a
changelog records what a header *said*.

**The rule, restated so it actually bites:** a reference to a file that is not in
the repo is a bug. `grep -n "\.md" *.js *.md` before claiming the docs are clean.

### A.24 Not done — queued
...
1. **Practice mode will not survive a real class period.** `maxInstances: 5` and
   ...
   once. The fix is the paragraph-pool cache (designed in the uncommitted
   `SCALE-PLAN.md` as "Problem 5"; the idea is pre-generate ~200 paragraphs per
   common problem-key-set, store in Firestore, serve at random) — but
...
3. **`firestore-rules.test.mjs` is wrong, not merely unrun.** Now committed (§2) so
   ...
   in §8 of this file is the practical substitute — `RULES-AUDIT.md`
   is not in the repo (§10).
