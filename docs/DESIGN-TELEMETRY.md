...
> ⚠️ **STILL OPEN, as of 2026-08-19 — but it was fixed and un-fixed once, so read this
> before re-attempting it.** Round 16 shipped a per-source split (`secondsLibrary` /
> `secondsSchool`), it broke twice in live use in two days, and it was **reverted**:
> `game.js` v3.30.0 and `learn.js` v2.15.0 write the flat triple again. The defect
> described below is therefore current, not historical. **`HANDOFF.md` §0.6 item 9 is
> required reading first** — it records which approach failed and why, and the reason
> for the revert was the calendar rather than the code. The v3.29.1 shape (per-page
> counters plus a base read once at page load) is the place to start, and it is in git
> history. Do not rebuild this from scratch without reading that item.
