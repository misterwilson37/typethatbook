...
console.log('\n─── D. ⚠️⚠️ EVERY ELEMENT A CONTROLLER GRABS STILL EXISTS ───');
// ⚠️⚠️ THE MIRROR OF PART B, AND IT IS HERE BECAUSE ROUND 116 SHIPPED THE BUG IT
// CATCHES. The mode pill replaced learn.html's "← Home" anchor — reasoning that
// a Library tab and a Home link were two records of one route — and that anchor
// was not a route at all: learn.js REPURPOSES it, so during a lesson it becomes
// the STOP control (`href='#'`, `onclick = stopLesson`) and reverts to
// index.html on the map.
//
// ⭐ THE COST WAS NOT THE LOST FEATURE. `backBtn.href = ...` threw on a null and
// **BOTH LESSON PAGES LOADED NOTHING AND CLICKED NOWHERE** — a page-fatal crash
// from deleting one anchor, found by Jake and by none of the 99 harnesses.
//
// ⚠️ PART B ASKS "is every BUTTON in the page wired?". This asks the opposite and
// the more dangerous question: **is every element the code GRABS still in the
// page?** An unwired button is a dead control; a missing element is a dead PAGE,
// because a top-level `const x = getElementById(...)` followed by any `x.foo =`
// takes the whole module down with it.
//
// ⚠️ GUARDED LOOKUPS ARE EXEMPT and deliberately so — `if (el)` or `el?.` says
// the author knows it may be absent, which is a different and legitimate claim.
{
    const PAIRS = [
        ['game.html', 'game.js'],
        ['learn.html', 'learn.js'],
        ['learn2.html', 'learn2.js'],
    ];
    for (const [htmlFile, jsFile] of PAIRS) {
        const html = read(htmlFile);
        const raw  = read(jsFile);
        const js   = withStrings(raw);
        const missing = [];
        // ⚠️⚠️ TOP-LEVEL BINDINGS ONLY, AND THAT PRECISION IS THE WHOLE CHECK.
        // A first draft matched every getElementById() anywhere and reported 28
        // false positives per page — controls these modules INJECT at runtime,
        // which of course are not in the static HTML. ⭐ THE FATAL SHAPE IS
        // NARROW: a binding taken at MODULE LOAD, before anything can have been
        // injected, and then written to. That is the one that takes the page
        // down before a student sees anything. A lookup inside a function fails
        // locally, at worst breaking one control.
        for (const m of raw.matchAll(
                /^(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*document\.getElementById\(\s*['"]([\w-]+)['"]\s*\)/gm)) {
            const [, name, id] = m;
            if (html.includes(`id="${id}"`) || html.includes(`id='${id}'`)) continue;
            // ⚠️ GUARDED LOOKUPS ARE EXEMPT. `if (el)`, `el?.` or `el &&` says the
            // author knows it may be absent — a different and legitimate claim.
            const guarded = new RegExp(
                `(if\\s*\\(\\s*!?${name}\\b|${name}\\s*\\?\\.|${name}\\s*&&)`).test(js);
            if (!guarded) missing.push(id);
        }
        ok(missing.length === 0,
           `\u26a0\u26a0 ${jsFile} grabs #${missing.join(', #')} unguarded and ` +
           `${htmlFile} no longer has ${missing.length === 1 ? 'it' : 'them'}. ` +
           `A top-level lookup plus any property write takes the WHOLE PAGE down ` +
           `— Round 116 did exactly this and both lesson pages went blank.`);
    }
}
