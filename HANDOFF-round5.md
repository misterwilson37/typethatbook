# HANDOFF — Round 5 (Mignon) — SUPERSEDED, KEPT ON PURPOSE

> **Round 6 (Noiseless) note:** this is the archived Round 5 handoff. The current
> one is `HANDOFF.md`. It is kept because Round 5 asked for exactly this and it did
> not happen — the Round 4 handoff was lost, and its §A history went with it. Round 6
> renamed this file into the repo rather than asking again.
>
> **What in here is still live:** §2 (an id is a label — never `parseInt` one) and
> §5 (all ten invariants) are load-bearing and are NOT fully restated in Round 6's
> handoff. §4.4's library table and §4.3's text-cleanup design are referenced by
> Round 6 §4.4–4.5 rather than copied.
>
> **What in here is now wrong:** §1's version table (six entries have moved on),
> §1.2 (`firebase-storage.rules` was deleted in Round 6), §4.1's first item
> (`admin.html` now has its title driven from `ADMIN_VERSION`), and §6's harness
> table — every one of those harnesses had a dead absolute path and could not run.
