...
console.log('\n═══ FULL LISTING: the two non-Standard-Ebooks books ═══');
for (const r of all) {
  const n = r.file.split('/').pop();
  if (!/pg7478|daddy/.test(n)) continue;
  console.log('\n### ' + n);
  r.chapters.forEach((c, i) => {
    const flag = (c.matter === 'body' && c.segments === 0) ? '   ← EMPTY body chapter' :
                 (/contents|^toby tyler$|gutenberg|licen[cs]e|transcriber/i.test(c.title) ? '   ← SUSPECT' : '');
    console.log(String(i).padStart(3) + '  id ' + String(c.id).padEnd(7) +
      c.matter.padEnd(6) + String(c.segments).padStart(4) + 'p  ' +
      c.srcFile.padEnd(30) + c.title.slice(0, 40) + flag);
  });
}
console.log('\n═══ front/back matter items on the SE books ═══');
for (const r of all) {
  const n = r.file.split('/').pop().replace(/\.epub$/, '');
  if (/pg7478|daddy/.test(n)) continue;
  const odd = r.chapters.filter(c => c.matter !== 'body');
  console.log('\n' + n + ':');
  odd.forEach(c => console.log('   ' + String(c.id).padEnd(7) + c.matter.padEnd(6) +
    String(c.segments).padStart(4) + 'p  ' + c.srcFile.padEnd(26) + c.title.slice(0,40)));
}
