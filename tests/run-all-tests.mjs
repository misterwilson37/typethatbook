...
const FAST = [
    ...
    ['featured-shelf-test.mjs', '\u26a0\ufe0f\u26a0\ufe0f ROADMAP 53 \u2014 FEATURED: newest first, random-untyped as the fallback when nothing is new. bookUploadedAtMs() is lifted and run against the same four timestamp shapes progressTimeMs() has; the cutoff-operator, stable-random-cache, and renderBooks-wiring assertions are mutation-verified, since a >= instead of > or a missing cache check would each be the kind of wrong that still renders, just wrong'],
