// run-all-tests.mjs v1.31.0 — Round 119 (Hammond): abandon-lock-test.mjs. 102 -> 103.
// run-all-tests.mjs v1.30.0 — Round 119 (Hammond): adaptive-arcade-test.mjs
// (ROADMAP 118a, the wiring half). 101 → 102; bumped in HANDOFF and README in
// the same edit, because docs-vs-repo-test.mjs C2 reads it.
// ⚠️ AND THE STAMP BELOW IS WRONG IN THE RECORD: Round 117 added
// arcade-scope-menu-test.mjs and wrote v1.29.0 over Round 116's v1.29.0, so two
// different registries share one version number and the header lies about which
// one a reader is holding. Left in place as the evidence rather than silently
// renumbered — see HANDOFF. ⭐ The lesson is the one Round 117 wrote for pass
// counts, one line over: a version is a claim, and nothing here reads it.
// run-all-tests.mjs v1.29.0 — Round 116 (Sun): shatter-shards-test.mjs. 98 → 99;
// bumped in HANDOFF and README in the same edit, because docs-vs-repo-test C2
// reads it.
// run-all-tests.mjs v1.29.0 — Round 117 (Corona): arcade-scope-menu-test.mjs (116g).
...
// library/ absent.
const FAST = [
    ["undefined-calls-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f every identifier reference resolves \u2014 WIDENED IN ROUND 124 from 13 modules to the whole root (59 files). Its FILES list had contained NO arcade module at all, against its own Round 114 rule (\u201cANY NEW .js OR ANY NEW PAGE, SAME COMMIT\u201d), so 27 modules had never been audited \u2014 which is how `tryScatter()` shipped with two call sites and no body, and how `spawnSpot()` sat in escape-board.js reading a constant that file had deleted on purpose"],
    ["account-names-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f A STUDENT'S NAME IS SAVED ON THEIR ACCOUNT \u2014 ONCE, AND ONLY THEIR REAL ONE. The student picker reads accounts and showed 178 of 178 as \u201c(no name)\u201d; names lived only on typing records. identityPlan() (real module) writes name and email once; Library, School and the Lessons beta save it in its OWN write before the day stamp; the rules accept only the sign-in token\u2019s own name and email, so no student can rename themselves as a classmate. \u26a0\ufe0f A deliberate second copy: Rule 9 explicitly overruled by Jake, 2026-09-25. Mutation-verified on the rules guard"],
    ["hand-guide-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f THE FINGER GUIDE REACHES EVERY KEY \u2014 AND SAYS SO WHEN IT CAN'T. Every key on every layout reaches a finger (real LAYOUTS and buildFingerMap). The REAL updateHandGuide(), in a jsdom page: a known dash activates the right pinky; a character the map doesn't know still reaches it via the lit key; each fallback or exit records a reason and warns ONCE with the code point. Built after the dash key's circle went missing in standard mode \u2014 the live cause was not visible from the code"],
    ["leaderboard-keep-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f A STUDENT'S INITIALS OUTLIVE THEM ON THE LEADERBOARD \u2014 AND NOTHING ELSE DOES. Runs the REAL keptRecordFrom(): initials, scores, their dates and school only, under a fresh id \u2014 never the account id, class, email or name; hidden students are not kept. Retention writes it BEFORE the purge; kept records off every board are pruned; the shared purgePlan() and Delete student\u2026 keep NOTHING, so a deletion request removes the initials too. Mutation-verified \u2014 including one planted in purgePlan() that the first draft of C5 missed"],
    ["teacher-change-stamp-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f A TEACHER'S CORRECTION SURVIVES THE STUDENT'S NEXT LOAD. stats-wal.js took the LARGER of the browser's log and the server for a whole week, so deleted minutes came back on every load \u2014 and on the same day game.js's flush wrote them into today's record. RULE 10: Part B reproduces it with the real student's numbers (507 s server, ~1,579 s log) on the REAL stats-wal.js. Part D checks Library, School and the Lessons beta carry a byte-identical gate. Mutation-verified"],
    ["auth-cleanup-sync-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f THE SIGN-IN ACCOUNT CLEANUP: every command in the reports page's How-to appears verbatim in scripts/auth-cleanup.py's docstring; the script's retention months, project id and protected admin emails match the site's; the policy states the same clock (records gone AND unused 24 months). Runs tests/auth-cleanup-test.py, which drives the REAL script against fakes. \u26a0\ufe0f The Cloud Shell credential steps are untested until Jake's first live run"],
    ["coppa-attestation-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f EVERY TEACHER CONFIRMS THEIR SCHOOL\u2019S COPPA CONSENT ONCE, AND THE RECORD CANNOT BE EDITED. Staff only, own record only, server-stamped, exact wording saved, `update: if false` \u2014 an attestation anyone could edit later is not evidence. Its own collection, so the staff record that guards role escalation is untouched. The statement says the SCHOOL approved; the teacher records it. Mutation-verified: an editable record and a teacher attesting for someone else both fail"],
    ["privacy-policy-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f EVERY PROMISE THE PRIVACY POLICY MAKES IS CHECKED AGAINST THE CODE THAT KEEPS IT. privacy.html is the COPPA online notice and the page a parent reads; a policy that drifts from the code is a false statement to a parent. Retention months are pinned to RETENTION_MONTHS in reports.html AND to SECURITY.md; \u201cother students see only initials\u201d to the leaderboard entry\u2019s fields; \u201cstudents can hide themselves\u201d to the board\u2019s filter; the guest claim to the absence of anonymous sign-in. Every page with a sign-in button must link the policy. Mutation-verified: a name added to the leaderboard, or retention changed in code alone, fails here. Unfilled address/phone placeholders print as NOTES, not failures"],
    ["retention-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f THE RETENTION PANEL REMOVES ONLY CHILDREN WHO ARE REALLY GONE. `activeDayLast` is stamped by Library and School but NOT the arcade, so trusting it would delete a child who played games last week \u2014 the verdict comes from the typing logs instead, one read per candidate on the document-id index. Runs the real retentionScan() against every kind of account: long-gone, arcade-only, brand new, empty, active, a TEACHER with a stale stamp, and the viewer. Mutation-verified three ways: stamp-only marks the arcade child for deletion; forgetting staff marks the teacher; bulk-deleting the review list is refused"],
    ["student-purge-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f DELETE THIS STUDENT DELETES EXACTLY ONE STUDENT, COMPLETELY. Runs the REAL purgePlan() and purgeExecute() from reports.html against an in-memory database holding TWO students: every record of the target goes, every record of the other survives, and the account record is deleted LAST so an interrupted run can be repeated. \u2b50\u2b50 Part A reads every collection the RULES declare and demands each be classified as student data or not \u2014 a new collection nobody classified fails the suite, so a new home for a child's data cannot escape deletion. Mutation-verified, including a query that drops its uid filter (the wrong child's records go, and B5 says so)"],
    ["self-hosted-fonts-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f NO PAGE LOADS FONTS FROM GOOGLE, AND EVERY LOCAL FONT EXISTS. Every page used to call fonts.googleapis.com, handing Google each visitor's IP \u2014 a third-party disclosure on a site used by children under 13. Round 135 self-hosts the three OFL families. \u2b50 The first attempt broke every page silently (the build shell has no brace expansion, the copy failed, the HTML edit ran anyway), which is exactly what part B catches. Also found shatter-preview.html naming Courier Prime and never loading it"],
    ["library-gate-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f ROADMAP 131a \u2014 THE LIBRARY LESSON GATE, AS JAKE RULED IT: under 15 WPM OR 80% offers lessons or another chance; a second bad minute locks, and while locked NOTHING IS RECORDED \u2014 seconds, characters, mistakes \u2014 because freezing only some of them either lets \u27f3 Recalculate rebuild the day from runs or manufactures a 100 WPM run that trips \ud83d\udea9. \u2b50 The unlock is PERFORMANCE, not attendance; it resets daily; a RELOAD DOES NOT. The gate functions are lifted from game.js and RUN, with only the DOM stubbed. Caught an off-by-one on its first run: the tick that completes a locked proving minute credited itself 100 ms. Four mutations, each red on the assertion written for it. \u26a0\ufe0f NOT lesson-gate-test.mjs, which tests the ARCADE gate in lesson-gate.js"],
    ["backspace-farm-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f A HELD BACKSPACE EARNED TEN MINUTES. `handleTyping()` stamped `lastInputTime` on its first line for every key, before looking at which one; a held key auto-repeats ~30 Hz, so the clock never went idle \u2014 and once the hold rewound to the start of the run every Backspace was a NO-OP that still stamped. Library now copies School's rule (only a real character press counts; learn.js was always immune) and additionally ignores `e.repeat`, which nothing in the repo had ever read. Part C replays the exploit through the REAL thresholds lifted from game.js: 600 s credited before, 0 after, and an auto-pause 5 s in. \u26a0\ufe0f RULE 10: a reproduction, not a recording \u2014 no farmed session was available. Part E pins the reports.html history flag (\u23f8, under 15 chars a minute of credited time) against REAL slow children from the 2026-09-22 traces so it cannot be tuned down onto them"],
    ["reports-identity-test.mjs", "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f PER-RUN DELETE NEVER WORKED ONCE. `sprintIdentity()` joined on U+0000 and the identity travels through a `data-run-id` attribute \u2014 the HTML parser is REQUIRED BY SPEC to replace U+0000 with U+FFFD, so the lookup returned -1 on every click since the button shipped. Every symptom falls out of that one character: the \u201csession may have changed\u201d notice (the -1 branch blaming a race that never happened), the day totals not moving (the write sits below the return), and nothing in the console (a handled branch). \u2b50 Round 128 also found, via the adversarial case in part B, that `escapeHtml()` escaped &, < and > but NOT quotes \u2014 correct in text position, catastrophic in the attributes it is actually used in, including `data-name` carrying student names. Part D pins that deleting a session removes its `.sprint-list` SIBLING too"],
    ["call-shape-test.mjs",      "\u26a0\ufe0f\u26a0\ufe0f\u26a0\ufe0f THE MIRROR OF undefined-calls-test: that one asks whether the NAMES exist, this one asks whether the ARGUMENT LISTS match. `platedText(ctx, o)` takes an options object and game-shatter.js\u2019s ping label called it with five positional arguments, so `o` was a string, `o.text` and `o.x` were undefined, the plate computed to NaN and NOTHING WAS EVER DRAWN \u2014 for three rounds, through two tunings and a reach rewrite, on the one control whose entire job is to show a word. Every identifier in that call resolved, so the reference audit was clean and correct and blind. \u2b50 TOO MANY ARGUMENTS IS THE FAILURE (JavaScript discards them silently); too few is a NOTE, because this codebase spells optional trailing parameters with an `== null` test rather than a default and the first draft went red on four innocent call sites"],
    ...
    ['shatter-shards-test.mjs',  '\u2b50 SHARDS, THE SECOND CABINET \u2014 the same game with a different motion model, offered beside Shatter so a class decides which is better. \u26a0\u26a0 Part A checks the two boards are INTERCHANGEABLE, because they share ONE VIEW and a missing method is a live cabinet throwing in a classroom that nothing else here would notice. \u26a0\u26a0 Part B asserts the INHERITANCE ITSELF \u2014 a drift board that quietly reimplemented tryKey or the split ladder would pass every behaviour test and be Rule 5 in a month; it overrides five methods, all motion. Part C: the director\u2019s lifetimeMs becomes SPEED rather than being thrown away, so the one number that knows how fast this child types still sets the pace. \u2b50 PART H FOUND THE REAL DEFECT: v1.0.0 let panes wrap forever, and over twelve seeds a 12 WPM typist took 15.2 hits against 10.5 for a student who did NOTHING \u2014 playing was worse than not playing, for exactly the children this app exists for, because clearing a word replaces one pane with two and a board with no exit meant a slow typist raised the density and then lived in it. Fixed with a wandering budget. \u26a0\u26a0 Part H also deliberately does NOT assert that typing protects you in proportion to skill, because it does not: the director refills to a fixed occupancy, so Shards is survive-by-luck and score-by-skill where Shatter is survive-by-skill. Written down rather than tuned away \u2014 ROADMAP 116e'],
    ...
    ['arcade-scope-menu-test.mjs', '\u26a0\u26a0 ROADMAP 116g \u2014 THE **WORDS** CONTROL MUST LOCK THE LESSON MENU, NOT JUST THE POOL. Jake, playing Deadline: *"if a student picks \'What I know so far\', it still gives him access to literally every lesson."* \u2b50 THE SETTING WAS LYING, which is worse than not offering it \u2014 a child chose the honest option and was handed Unit 5. \u26a0\u26a0 THE EXCEPTION HAS MORE ASSERTIONS THAN THE RULE, because the naive fix fails it QUIETLY: arcadeWindow() with no progress returns 0, so a brand-new child meets a legal, non-empty and entirely wrong menu of ONE. Part D is the off-by-N that only appears once a lesson drops out of PLAYABLE \u2014 the cap is a LESSONS index and the <option> value is a PLAYABLE one, and comparing them would offer work past the window to exactly the students whose course has a gap in it. \u26a0 Drives the real 47-lesson corpus (Rule 10); mutation-verified by restoring the shipped behaviour, which turns four assertions red'],
    ['typing-calibrator-test.mjs', '\u26a0\u26a0 ROADMAP 118a \u2014 MEASURE THE CHILD EVERY RUN INSTEAD OF ASKING THEM. Jake: *"When I choose 100 (what I type), deadline is impossible \u2014 even for me."* \u2b50 GAME WPM AND PROSE WPM ARE DIFFERENT QUANTITIES WEARING THE SAME NAME \u2014 peekNext()\'s own measurement says a 100 WPM typist manages 44 in-game, so the picker was demanding 2.3x what is physically available. Measures BURST SPEED and ACQUISITION separately, because more panes make a fast reader faster and a hunting child slower \u2014 the per-student answer to the MIN_ON_SCREEN=3 experiment that collapsed the corpus sweep to 53.2% as a global. \u26a0\u26a0 PART B IS THE CLASSROOM FAILURE, NOT THE LAB ONE: a child who stares out of the window for twelve seconds must not set the difficulty for the run, so samples are capped and the estimate is a MEDIAN. \u26a0\u26a0 PART F IS WHAT THIS FILE EXISTS FOR \u2014 not "does it adapt" but "can a slow child end up worse off than today", including a ratchet test where one fluke fast word must not drag a struggling student upward. \u26a0 Drives a real GameDirector (Rule 10; Round 117 learned why). \u2b50 It also caught its own author counting n characters across n-1 inter-key gaps \u2014 a 25% overstatement on five-letter words, i.e. worst for the youngest children'],
    ['adaptive-arcade-test.mjs', '\u26a0\u26a0 ROADMAP 118a, THE WIRING HALF \u2014 Round 118 built the calibrator and NOTHING CALLED IT, and all 101 harnesses stayed green over an engine that was dead code. \u2b50\u2b50 PART A IS THE TRAP THE ONE-LINE INSTRUCTION WAS ABOUT TO SPRING: `arcadeConfig()` is not Shatter\u2019s config \u2014 isFreePlay() routes Shatter, Shards, Escape Key AND full-scope Deadline through it, and only Shatter feeds the calibrator, so the other two would have run at the pre-comfort seed of 8 WPM FOR EVER (2400ms spawn interval \u2192 6000ms, 16.8s lifetime \u2192 42s on a 20 WPM gate). \u2b50 THE SEED IS THE FLOOR *BECAUSE A MEASUREMENT IS COMING*; where none is coming it is a broken game, not an easy one. Fixed by requiring a calibrator the VIEW supplies \u2014 the only honest signal, since \u201cno samples yet\u201d is exactly what a child who froze looks like and Jake ruled that child gets the floor. \u26a0\u26a0 PART B IS THE GRADED PATH: missionConfigFromRun() must never grow this field, asserted on the SOURCE, because the 990-trial clearability sweep is a promise about a FIXED gate and a run whose pace moved mid-flight would void it silently and per-student. \u26a0 PART D PINS A RULING, NOT A BEHAVIOUR: pieces are not calibration samples \u2014 a piece is born where the student is already looking, so folding them in drags acquisition down, raises onScreenTarget and puts MORE panes in front of the hunting child this design exists to protect. \u26a0\u26a0 PART E IS ONE IDENTIFIER: the calibrator takes `now` and never `bNow`, which runs at 22% during the shatter slowdown and would report a child up to 4.5\u00d7 faster than they are \u2014 one line below a board.spawn() that correctly takes the board clock. Part G is Rule 11: the number never reaches a screen. \u2b50 Mounts the REAL view and drives the REAL key handler (Rule 10); its own rAF fake had to become a QUEUE, because game-chrome\u2019s countdown runs a second loop and a one-slot fake let them clobber each other'],
    ['abandon-lock-test.mjs',   '\u26a0\u26a0 A STUDENT MUST ALWAYS BE ABLE TO LET GO OF THE WORD THEY ARE ON. A child told Jake there was no escape key, and he hit it the same day: *\u201cif I think I\u2019m typing one word, but I\u2019m actually typing another, there\u2019s no way to get out of it... when the sky is covered in words, you can\u2019t tell where you\u2019re missing.\u201d* \u2b50\u2b50 THE CAUSE IS NOT THE ONE IT LOOKS LIKE, AND THIS FILE\u2019S OWN MUTATION TEST CORRECTED ITS AUTHOR: the first draft said Escape had become UNREACHABLE because game-chrome.js binds it to pause with stopPropagation(). \u26a0\u26a0 stopPropagation() STOPS OTHER TARGETS, NOT OTHER LISTENERS ON THE SAME ONE \u2014 that is stopImmediatePropagation() \u2014 and both handlers sit on `window`, so ESCAPE DID BOTH THINGS AT ONCE: released the lock AND paused the game. \u2b50 ALIVE AND UNUSABLE IS A WORSE DEFECT THAN DEAD: you could not abandon a word without freezing the game, and the freeze is the only half a student can see. \u26a0\u26a0 AND IN DEADLINE IT WAS A TRAP \u2014 every key goes to `locked`, and the auto-lock skips any target with typed > 0 as \u201calready someone\u2019s business\u201d, so a student who locked the wrong word was STUCK IN IT UNTIL IT LANDED and the half-typed word could never be re-acquired. Backspace/Delete now releases the lock AND wipes the progress \u2014 the wipe is the half that matters, because a pane wanting a letter the student cannot see charges every restart as a mistake with nothing on screen to explain why. \u26a0 PART D IS THE HALF THAT WOULD HAVE CAUGHT THE ORIGINAL: no hint may name a key its view does not handle. The code was one side of the failure; four sentences on screen promising Esc were the other, in a hint a child reads precisely when they are stuck. \u26a0 The harness\u2019s own first draft went red against correct code by pressing Escape (which pauses) and then complaining the game was paused'],
