...
// ⚠️ THE REFUSAL CONSTANTS ARE LIFTED FROM THE SHIPPED FILE, NOT RETYPED HERE.
// SCHOOL_SESSIONS_FROM, MAX_DROP_FRACTION and MAX_DROP_FLOOR_SECONDS (v2.17.0)
// are module-scope consts that buildRebuildPlan() closes over. Copying their
// VALUES into this harness would let reports.html's date drift while the test
// went on passing against the old one — which is exactly how a guard becomes
// decoration. `formatTime` is stubbed because the refusal messages call it.
function liftConst(src, name) {
    const m = new RegExp('const\\s+' + name + '\\s*=\\s*([^;]+);').exec(src);
    return m ? `const ${name} = ${m[1]};` : '';
}
const REFUSAL_CONSTS =
...
function makePlanner(reportData, reconState, today) {
    return new Function('reportData', 'reconState', '__today', `
        ...
        function formatTime(s) { return String(s) + 's'; }
        ${REFUSAL_CONSTS}
        ${uidMapBody}
        ${planBody}
...
function fixture() {
    ...
    const reconState = { results: [
        ...
        // 24 minutes exist ONLY in the daily log. REFUSED BY DATE.
        { idx: 0, uid: 'u_abe', date: '2026-08-17', li: 2, hasLog: true, docId: 'u_abe_2026-08-17',
          logSeconds: 1584, logChars: 2600, logMistakes: 40, sess: S(111, 190, 4, 1), error: null },
        // ⚠️ Bea 8/19 is TODAY... no: 8/18, a date the date-guard allows, but the
        // drop is 88% of the day. REFUSED BY PROPORTION. This is the guard that
...
if (planBody && uidMapBody) {
    ...
    // write that would have destroyed twenty-four minutes of real graded School
    // time, because School kept no session records before 2026-08-18 and the
    // day was not EMPTY (it had 1m 51s of Library sessions), so the existing
    // never-zero refusal did not fire. A PARTIAL record defeats an emptiness
    // check and is more dangerous than no record at all.
    const whyFor = (uid, date) => {
